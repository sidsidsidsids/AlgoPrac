...
for _ in range(K):
    ...
    A_name, A_parent = A.split('/')[-1], A.split('/')[-2]
    B_name = B.split('/')[-1]
    A_folder = folder_map[A_name]
    B_folder = folder_map[B_name]
    A_parent_folder = folder_map[A_parent]
    for subfolder in A_folder.subfolders:
        B_folder.add_folder(subfolder)
    for subfile in A_folder.subfiles:
        B_folder.add_file(subfile)
    A_parent_folder.subfolders.pop(A_name)
    folder_map.pop(A_name)
